...
### 5. Local RAG (Optional)

You can enable local retrieval to ground generation from PDF knowledge sources.

1. In **AI Configuration** -> **RAG Settings (Local)**, enable **Local RAG**.
2. Keep defaults for first run:
   * Collection: `synthesizer_default`
   * Qdrant URL: `http://localhost:6333` (or use in-memory mode in tests)
   * Embedding model: `BAAI/bge-small-en-v1.5`
3. Click **Ingest PDF** and select a document from `examples/` or your own files.
4. Start generation as normal. Prompt construction will automatically inject retrieved context when available.
